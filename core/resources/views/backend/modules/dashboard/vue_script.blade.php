...
{{--                axios.get(`${domainPath}api/dashboard/`)--}}
{{--                    .then(response => {--}}
{{--                        // Animate integer-based values like orders and statuses--}}
{{--                        this.animateValue('todayOrders', 0, response.data.todayOrders, 2000);--}}
{{--                        this.animateValue('totalOrders', 0, response.data.totalOrders, 2000);--}}
{{--                        this.animateValue('pendingOrders', 0, response.data.pendingOrders, 2000);--}}
{{--                        this.animateValue('completeOrders', 0, response.data.completeOrders, 2000);--}}

{{--                        // Animate currency-based values like sales--}}
{{--                        this.animateValue('todaySales', 0, response.data.todaySales, 2000, true);--}}
{{--                        this.animateValue('totalSales', 0, response.data.totalSales, 2000, true);--}}
...
{{--                if (value >= 1000) {--}}
{{--                    return Math.round(value / 1000) + 'k';  // Remove decimal places for values >= 1000--}}
{{--                } else {--}}
{{--                    return value.toFixed(2);  // Keep two decimal places for smaller values--}}
{{--                }--}}
{{--            },--}}
{{--          --}}
{{--            getImageUrl(relativePath) {--}}
{{--                if (relativePath) {--}}
{{--                    return domainPath + relativePath; // Return the full URL if relativePath exists--}}
{{--                } else {--}}
{{--                    return domainPath + 'theme/frontend/assets/img/default/book.png'; // Return default image URL if no relativePath--}}
{{--                }--}}
{{--            },--}}
{{--            animateValue(refName, start, end, duration, isCurrency = false) {--}}
{{--                let startTimestamp = null;--}}
...
{{--                    // Apply formatting based on whether it's currency or an integer value--}}
{{--                    if (isCurrency || value >= 1000) {--}}
{{--                        this[refName] = this.formatWithK(value);  // Format with 'k' for large numbers--}}
{{--                    } else {--}}
{{--                        this[refName] = Math.floor(value);  // Round down for integer values below 1000--}}
{{--                    }--}}
