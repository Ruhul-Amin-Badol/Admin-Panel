...
class BlogController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Process and sync categories (if provided)
        if (!empty($validatedData['category_ids'])) {
            $categoryIds = [];
            foreach ($validatedData['category_ids'] as $categorySlugOrName) {
                $category = Category::where('slug', $categorySlugOrName)
                ...
                if (!$category) {
                    ...
                    $category = Category::create([
                        'name' => trim($categorySlugOrName),
                        'slug' => Str::slug(trim($categorySlugOrName), '_'),
                    ]);
                }
        ...
        // Process and sync tags (if provided)
        if (!empty($validatedData['tags'])) {
            $tagIds = [];
            foreach ($validatedData['tags'] as $tagSlugOrName) {
                $tag = Tag::where('slug', $tagSlugOrName)
                    ->orWhere('name', $tagSlugOrName)
                    ->first();

                if (!$tag) {
                    // Create new tag if not found
                    $tag = Tag::create([
                        'name' => trim($tagSlugOrName),
                        'slug' => Str::slug(trim($tagSlugOrName), '_'),
                    ]);
                }
        ...
        if (!empty($validatedData['tags'])) {
            ...
            foreach ($validatedData['tags'] as $tagNameOrId) {
                if (is_numeric($tagNameOrId)) {
                    ...
                    $tag = Tag::firstOrCreate(['name' => trim($tagNameOrId)]);
                    $tagIds[] = $tag->id;
                }
            }
            $blog->tags()->sync($tagIds);
